...
def main() -> int:
    ...
    try:
        ...
        assert_true("easy-panel-shell" in app_js, "首页导航应整合 Easy Panel 风格首页结构")
        assert_true("Easy Panel" in app_js, "首页导航顶部应复刻 Easy Panel 标题")
        assert_true("农历三十" in app_js, "首页导航顶部应复刻农历标签")
        assert_true("19.1°C" in app_js, "首页导航顶部应复刻天气温度显示")
        assert_true("easy-panel-icon-actions" in app_js, "首页导航顶部应复刻右侧图标工具区")
        assert_true("nav-settings-tabs" in app_js, "首页设置应使用 Easy Panel 风格分栏设置面板")
        assert_true("background_effect" in app_js, "首页导航应支持 Easy Panel 风格全局背景效果")
        assert_true("header_modules" in app_js, "首页导航应支持顶部模块开关和排序配置")
        assert_true("search_engine" in app_js, "首页导航应支持搜索引擎配置")
        assert_true("iconfont_url" in app_js, "首页导航应支持自定义图标库配置")
        assert_true("nav-backup-export" in app_js, "首页导航应支持配置备份导出")
        assert_true("nav-ai-command" in app_js, "首页导航应整合 AI 命令栏入口")
        assert_true("状态模块" not in app_js, "首页导航不应显示额外状态模块")
        assert_true("renderDashboardWidgets()" not in app_js, "首页导航不应插入状态模块面板")
        ...
        assert_true("easy-panel-aurora" in styles_css, "首页导航应提供 Easy Panel 风格动态背景")
        assert_true("easy-panel-topbar" in styles_css, "首页导航应使用 Easy Panel 横向顶部工具条")
        assert_true("easy-panel-lunar" in styles_css, "首页导航顶部应提供农历徽标样式")
        assert_true("easy-panel-icon-actions" in styles_css, "首页导航顶部应提供右侧图标工具样式")
        assert_true("easy-panel-canvas" in styles_css, "首页导航内容应铺在 Easy Panel 背景画布上")
        assert_true("nav-settings-tabs" in styles_css, "首页设置面板应使用分栏布局")
        assert_true("easy-panel-search" in styles_css, "首页导航应提供 Easy Panel 风格搜索栏")
        ...
        assert_true('["dashboard", "containers", "images", "volumes", "compose"].includes(state.tab)' in app_js, "首页、容器、镜像库、卷和 Compose 页面应隐藏顶部用户栏")
        ...
        assert_true("background_effect" in nav_prefs["prefs"], "首页导航偏好应包含背景效果")
        assert_true("header_modules" in nav_prefs["prefs"], "首页导航偏好应包含顶部模块")
        _, nav_saved = client.request("PUT", "/api/dashboard/nav", {"title": "我的导航", "density": "compact", "background_effect": "aurora", "search_engine": "bing", "groups": {"Docker": {"collapsed": True, "color": "#16a34a"}}})
        assert_true(nav_saved["prefs"]["title"] == "我的导航", "首页导航标题应可保存")
        assert_true(nav_saved["prefs"]["search_engine"] == "bing", "首页导航搜索引擎应可保存")
