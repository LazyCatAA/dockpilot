...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260518-9" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260518-9" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        assert_true("/vendor/codemirror/lang-yaml.bundle.mjs" in page, "首页应加载本地 CodeMirror YAML 编辑器")
        assert_true("https://esm.sh" not in page, "CodeMirror 不应依赖外网 CDN")
        _, app_js = client.request("GET", "/app.js")
        _, cm_yaml = client.request("GET", "/vendor/codemirror/lang-yaml.bundle.mjs")
        _, cm_process = client.request("GET", "/node/process.mjs")
        assert_true("export" in cm_yaml and "yaml" in cm_yaml, "本地 CodeMirror YAML bundle 应可访问")
        assert_true("export default process" in cm_process, "本地 CodeMirror process shim 应可访问")
        ...
        assert_true(".cm-gutters" in page, "CodeMirror 应启用原生行号栏")
