...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260520-69" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260520-69" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true("bookmark-icon-actions" in app_js, "书签图标应提供智能匹配操作区")
        ...
        assert_true("cardBackgroundUpload" in app_js, "书签卡片应支持背景图上传")
        assert_true("cardModal" in app_js, "书签卡片应使用弹窗添加和编辑")
        assert_true("bookmark-editor-modal" in app_js, "书签编辑窗口应使用独立弹窗样式")
        assert_true("bookmark-editor-shell" in app_js, "书签编辑窗口应使用参考图式窄版结构")
        assert_true("bookmark-editor-section" in app_js, "书签编辑窗口应按参考图拆分表单区块")
        assert_true("card-icon-scale" in app_js, "书签编辑窗口应支持图标缩放滑块")
        assert_true("card-background-pick" in app_js, "书签编辑窗口应支持背景图上传按钮")
        assert_true("background_opacity" in app_js, "书签编辑窗口应支持背景透明度")
        assert_true("show_background" in app_js, "书签编辑窗口应支持背景显示开关")
        assert_true("is_public" in app_js, "书签编辑窗口公开开关应提交真实字段")
        assert_true("bookmark-icon-actions" in app_js, "书签编辑窗口应保留智能匹配入口")
        assert_true("bookmark-editor-upload" in app_js, "书签编辑窗口应保留背景图上传入口")
        ...
        assert_true("width: min(470px, calc(100vw - 40px))" in styles_css, "书签编辑窗口应按参考图收窄")
        ...
        assert_true("padding: 10px 10px 90px 66px" not in styles_css and "padding: 10px 10px 90px 72px" not in styles_css, "手机端内容区不应继续给已隐藏的左侧菜单预留宽度")
        assert_true("padding-left: 62px" not in styles_css, "小屏手机内容区应保持水平居中，不应保留左侧菜单空隙")
        assert_true("padding: calc(4px + env(safe-area-inset-top)) 10px calc(90px + env(safe-area-inset-bottom)) 10px" in styles_css, "手机端内容区应使用左右对称 padding")
        assert_true("@media (max-width: 1024px) {\n  .responsive-baseline {\n    grid-template-columns: 156px minmax(0, 1fr);" in styles_css, "1024px 窄桌面布局列宽应与桌面侧边栏同步，避免侧栏遮挡内容")
        assert_true("@media (max-width: 980px) and (min-width: 721px)" in styles_css and "grid-template-columns: 70px minmax(0, 1fr)" in styles_css, "平板窄屏布局列宽应与收窄侧边栏同步，避免侧栏遮挡内容")
        ...
        assert_true("@media (min-width: 1920px)" in styles_css, "4K/超宽屏首页导航应提供专用布局规则")
        assert_true("max-width: min(3000px, calc(100vw - 260px))" in styles_css, "4K/超宽屏首页导航画布应放宽，避免内容缩在中间小块")
        assert_true("grid-template-columns: repeat(auto-fit, minmax(430px, 1fr))" in styles_css, "4K/超宽屏首页书签分组应自适应多列铺开")
        assert_true("grid-column: 1 / -1" in styles_css, "4K/超宽屏首页书签库标题应横跨所有分组列")
