...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260519-57" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260519-57" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true("contextMenuPositionForCard" in app_js, "书签右键菜单应锚定到书签卡片右下角")
        assert_true("getBoundingClientRect()" in app_js, "书签右键菜单定位应读取书签卡片边界")
        assert_true("openCardContextMenuFromNode" in app_js, "书签右键菜单应有统一打开逻辑")
        assert_true("pointerup" in app_js, "书签右键菜单应兼容真实鼠标右键事件")
        ...
        assert_true("linear-gradient(135deg, #1a2d73" in styles_css, "首页导航应使用参考图式深蓝画布")
