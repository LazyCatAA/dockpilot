...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260518-5" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260518-5" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true("code-line-number" in app_js, "Compose 编辑器应显示代码行号")
        ...
        assert_true("grid-template-columns: 38px minmax(0, 1fr)" in styles_css, "Compose 代码行应有紧凑固定行号列")
        assert_true("line-height: 1.48" in styles_css, "Compose 编辑器行距应更紧凑")
        assert_true("-webkit-text-fill-color: transparent !important" in styles_css, "Compose 编辑器输入层应透明以显示高亮层")
        highlight_start = styles_css.find(".compose-reference-layout .code-highlight {")
        highlight_block = styles_css[highlight_start:styles_css.find(".compose-reference-layout .code-input {", highlight_start)]
        assert_true("visibility: hidden" not in highlight_block, "Compose 高亮层不应隐藏代码内容")
        assert_true("z-index: 4" in highlight_block, "Compose 高亮层应位于输入层上方，确保代码可见")
        assert_true(".compose-reference-layout .code-highlight .yaml-key" in styles_css, "Compose 高亮层应显式设置 YAML 颜色")
