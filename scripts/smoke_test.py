...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260518-4" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260518-4" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true(".compose-reference-layout .code-highlight .code-line-content" not in styles_css, "Compose 高亮内容不应被隐藏")
        assert_true("visibility: hidden" not in styles_css[styles_css.find(".compose-reference-layout .code-highlight"):styles_css.find(".compose-reference-layout .code-input")], "Compose 高亮层不应隐藏代码内容")
