...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260518-6" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260518-6" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true("composeLineNumbers" in app_js, "Compose 编辑器应使用独立行号栏，避免遮挡选择光标")
        ...
        assert_true("compose-line-numbers" in styles_css, "Compose 编辑器应提供独立行号栏")
        assert_true("line-height: 1.38" in styles_css, "Compose 编辑器行距应更紧凑")
        assert_true("-webkit-text-fill-color: #d8e6f7" in styles_css, "Compose 编辑器真实输入文字应可见，保证选择和光标可用")
