...
def main() -> int:
    ...
    try:
        ...
        assert_true("nav-searchbar" in styles_css, "导航页应提供顶部居中搜索样式")
        assert_true("nav-settings-button" in styles_css, "导航页右上角应提供设置按钮样式")
        ...
        assert_true('["dashboard", "containers", "images", "volumes", "compose"].includes(state.tab)' in app_js, "导航、容器、镜像库、卷和 Compose 页面应隐藏顶部用户栏")
        ...
        _, nav_saved = client.request("PUT", "/api/dashboard/nav", {"title": "我的导航", "density": "compact", "web_search_engine": "bing", "groups": {"Docker": {"collapsed": True, "color": "#16a34a"}}})
