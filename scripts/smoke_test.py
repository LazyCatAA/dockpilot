...
def main() -> int:
    ...
    try:
        ...
        assert_true("app.js?v=20260518-1" in page, "首页应引用新的前端资源版本，避免浏览器缓存旧 app.js")
        assert_true("styles.css?v=20260518-1" in page, "首页应引用新的样式资源版本，避免浏览器缓存旧 styles.css")
        ...
        assert_true('data-action="compose-apply-ai" ${state.compose.aiContent ? "" : "disabled"}>应用修复</button>' in app_js, "Compose 右侧应用修复按钮应应用 AI 预览内容")
        ...
        assert_true("grid-template-columns: 44px minmax(0, 1fr)" in styles_css, "Compose 代码行应有固定行号列")
